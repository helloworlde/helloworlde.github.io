...
![OpenWrt 虚拟机在 PVE 中创建并导入镜像](https://img.hellowood.dev/picture/homelab-openwrt-esxi-ima-download.png)
...
![OpenWrt 在 PVE 中创建虚拟机的初始配置界面](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-configuration.png)
...
![PVE 界面上传 OpenWrt 镜像文件到 ISO 存储](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-upload-img.png)

等待上传完成，记录上传后的地址，即 target file 后面的路径，需要在导入时使用

![PVE 界面显示 OpenWrt 镜像上传完成及目标文件路径](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-upload-img-result.png)
...
![在 PVE 中将 OpenWrt 镜像转换为虚拟磁盘](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-convert-img-to-disk.png)

- 将硬盘添加到虚拟机

待导入完成后，会在虚拟机中显示未使用的磁盘

![在 PVE 虚拟机设置中添加硬盘并修改引导顺序](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-add-disk-to-vm-0.png)

选择该磁盘，修改 "总线/设备" 为 SATA，然后点击添加

![在 PVE 虚拟机设置中添加硬盘并修改总线为 SATA](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-add-disk-to-vm-1.png)
...
![PVE 虚拟机设置界面修改引导顺序为 sata0 磁盘](https://img.hellowood.dev/picture/homelab-openwrt-pve-init-set-boot-order.png)
...
![OpenWrt 虚拟机网络配置完成后的登录界面](https://img.hellowood.dev/picture/homelab-openwrt-esxi-login.png)
