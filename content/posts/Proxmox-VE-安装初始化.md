...
![使用 Rufus 将 Proxmox VE ISO 镜像写入 U 盘](https://img.hellowood.dev/picture/homelab-pve-install-write-iso.png)

- 插入主机并启动

启动后选择安装 Proxmox VE

![Proxmox VE 安装界面选择开始安装](https://img.hellowood.dev/picture/homelab-pve-install-start-install.png)

- 设置 IP 地址

IP地址用于后续访问，可以通过DHCP获取，也可以设置为固定的 IP

![Proxmox VE 安装界面设置静态 IP 地址](https://img.hellowood.dev/picture/homelab-pve-install-set-ip.png)

- 安装完成

安装完成后，会提示重启

![Proxmox VE 安装完成后的系统重启提示界面](https://img.hellowood.dev/picture/homelab-pve-install-completed.png)

- 登录

重启完成后，在命令行会提示访问的地址，默认端口是 8006，

![Proxmox VE 安装完成后的 Web 登录界面](https://img.hellowood.dev/picture/homelab-pve-install-login-page.png)

通过 HTTPS 协议访问；如 [https://192.168.17.129:8006](https://192.168.17.129:8006)

![Proxmox VE 安装完成后的 HTTPS 登录页面](https://img.hellowood.dev/picture/homelab-pve-install-web-login-page.png)
