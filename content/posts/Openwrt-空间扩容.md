---
description: "解决 OpenWrt 默认 100M 空间不足问题，通过 ESXi 调整虚拟机硬盘大小、使用 cfdisk 创建分区及 block-mount 挂载 ext4 实现扩容。"
title: Openwrt-空间扩容
keywords:
  - "OpenWrt"
  - "ESXi"
  - "cfdisk"
  - "block-mount"
  - "ext4"
  - "overlay"
  - "partition"
  - "disk expansion"
slug: "openwrt-storage-expansion"
aliases:
  - "/posts/openwrt-空间扩容/"
type: post
date: 2022-07-23T11:32:08+08:00
lastmod: 2026-03-22
tags:
  - Esxi
  - OpenWrt
  - HomeLab
featured: true
---

Openwrt 默认的空间只有 100M，安装一些软件后空间就被用完了，因此需要对 Openwrt 的硬盘进行扩容

有多种扩容方式，如新增一块硬盘，以USB挂载的方式扩容；或者修改虚拟机硬盘文件大小的方式扩容；此次通过修改虚拟机硬盘文件大小的方式扩容，这种方式适合新创建的虚拟机

## 修改硬盘文件大小

通过 Esxi 控制台，直接修改挂载的硬盘大小

- 在 Esxi 修改硬盘大小

![Esxi 控制台修改 OpenWrt 虚拟机硬盘大小](https://img.hellowood.dev/picture/homelab-openwrt-esxi-disk-size.png)

## 创建新的分区

1. 安装 `cfdisk`

需要使用 `cfdisk` 创建新的分区，所以需要安装 `cfdisk`

```bash
opkg update
opkg install cfdisk
```

2. 使用 `cfdisk`创建分区

命令行输入 `cfdisk` ，会进入到管理分区页面；可以看到，`Free Space`是新添加的硬盘大小

- 选择 `Free Space` 后选择 `New`，创建新的分区
- 输入 `Partition Size`为想要的大小
- 如`3.9G`，然后回车，此时可以看到挂载了新的分区 `/dev/sda3`

![cfdisk 界面显示新创建的 sda3 分区](https://img.hellowood.dev/picture/homelab-openwrt-esxi-disk-new-partition-a-1.png)

选择 `/dev/sda3`，使用 Tab 选择 `Write`并输入 `yes`确认
![Openwrt 磁盘管理界面创建新分区并写入](https://img.hellowood.dev/picture/homelab-openwrt-esxi-disk-new-partition-a-2.png)

3. 格式化分区

格式化新增的 `/dev/sda3`

```bash
mkfs.ext4 /dev/sda3
```

## 挂载扩容的空间

- 安装 `block-mount`

挂载新建的分区，需要使用 `block-mount`挂载点软件，安装完成后重启 Openwrt

```bash
opkg update
opkg install block-mount
reboot
```

重启完成后访问 Web 页面的 `系统`-`挂载点`

![Openwrt 挂载点页面配置外部 overlay 扩容](https://img.hellowood.dev/picture/homelab-openwrt-esxi-disk-new-partition-a-mount-3.png)

选择`新增`，选择刚才创建的分区`/dev/sda5`，挂载点为`作为外部 overlay 使用`，并启用

![Openwrt 挂载点设置界面配置外部 overlay](https://img.hellowood.dev/picture/homelab-openwrt-esxi-disk-new-partition-a-mount-1.png)

保存并启用修改的配置，重启 Openwrt，进入挂载点，即可看到`/overlay`空间已经扩容

![Openwrt 新增分区挂载为外部 overlay 扩容](https://img.hellowood.dev/picture/homelab-openwrt-esxi-disk-new-partition-a-mount-2.png)

## 参考文档

- [Extroot configuration](https://openwrt.org/docs/guide-user/additional-software/extroot_configuration)
