...
![在 PVE 中添加 PCI 设备直通网卡到 OpenWrt](https://img.hellowood.dev/picture/homelab-openwrt-pve-switch-add-pci-devices.png)
...
![OpenWrt 网络接口配置界面中设置 br-lan 网桥端口](https://img.hellowood.dev/picture/homelab-openwrt-pve-switch-config-network-0.png)

在网桥端口中，将 `eth0`, `eth1`,`eth2`,`eth3`全部选中，作为网桥的桥接端口；然后点击保存并应用，这样就可以将 OpenWrt 作为交换机使用了

![OpenWrt 网桥配置界面将四个网口设为桥接端口](https://img.hellowood.dev/picture/homelab-openwrt-pve-switch-config-network-1.png)
...
![OpenWrt 桥接模式下 NAS 与电脑 2.5G 测速结果](https://img.hellowood.dev/picture/homelab-openwrt-pve-switch-speed-test.png)
