...
![在 Cloudflare Zero Trust 控制台创建新 Tunnel 的界面](https://img.hellowood.dev/picture/homelab-cloudflare-tunnel-init-create-tunnel.png)

![在 Cloudflare Zero Trust 控制台创建 Tunnel 并输入名称](https://img.hellowood.dev/picture/homelab-cloudflare-tunnel-init-name-tunnel.png)

## 安装配置 Cloudflare Tunnel

Tunnel 支持二进制包或者 Docker 容器的方式进行安装；选择相应的平台，会生成安装命令，在命令后执行该命令即可

![在 Ubuntu 终端执行 Cloudflare Tunnel 安装命令](https://img.hellowood.dev/picture/homelab-cloudflare-tunnel-init-install.png)
...
![Cloudflare Tunnel 路由器配置界面设置内网服务](https://img.hellowood.dev/picture/homelab-cloudflare-tunnel-init-router-config.png)
...
![IPv4 与 IPv6 连接 Cloudflare 边缘节点数量对比图](https://img.hellowood.dev/picture/homelab-cloudflare-tunnel-metrics-ipv4-and-ipv6-compare.png)
